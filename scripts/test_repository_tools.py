...
class RepositoryToolTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        package = self.root / "engineering" / "example-skill"
        package.mkdir()
        ...
        (self.root / "SKILLS.md").write_text(
            ...
            "| --- | --- | --- | --- |\n"
            "| `engineering/example-skill` | curated | none | fixture |\n",
            encoding="utf-8",
        )
    ...
    def test_rejects_missing_root_doc_unknown_binary_and_inventory_mismatch(self) -> None:
        ...
        (self.root / "engineering" / "example-skill" / "asset.bin").write_bytes(b"\x00\x01")
    ...
    def test_installer_refuses_source_symlink_and_overwrite(self) -> None:
        ...
        destination.mkdir()
        args = ["engineering/example-skill", "--root", str(self.root), "--dest", str(destination)]
        self.assertEqual(INSTALLER.main(args), 0)
        self.assertEqual(INSTALLER.main(args), 3)
        alias = self.root / "engineering" / "alias"
        alias.symlink_to(self.root / "engineering" / "example-skill", target_is_directory=True)
        alias_args = ["engineering/alias", "--root", str(self.root), "--dest", str(destination)]
